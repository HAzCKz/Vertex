...
ITER=210 WARMUP=10 MEASURE_RESOLVE=0 \
node teste-node/time/test_time_03_von_network_ops_warmup_p99.js

ENV OPCIONAIS:
- WARMUP=2              -> descarta as 2 primeiras iterações das estatísticas
- MEASURE_RESOLVE=1     -> mede resolveDidOnLedger(issuer/holder) após register NYM
*/
...
(async () => {
  ...
  const stats_all = {}; // opcional: bruto incluindo warmup (para debug)

  console.log("============================================================");
  console.log("⏱️  TESTE TIME 03: benchmark SSI (Von) + WARMUP + p99 + stddev");
  console.log("Config:", {
    GENESIS_FILE,
    ITER,
    WARMUP,
  ...
  for (let i = 1; i <= ITER; i++) {
    ...
    try {
      ...
      await T("importDidFromSeed(TRUSTEE)", async () => issuer.importDidFromSeed(TRUSTEE_SEED));

      const [issuerDid, issuerVerkey] = await T("createOwnDid(issuer)", async () => issuer.createOwnDid());
      const [holderDid, holderVerkey] = await T("createOwnDid(holder)", async () => holder.createOwnDid());

      await T("registerDidOnLedger(issuer)", async () =>
      ...
      if (MEASURE_RESOLVE && hasMethod(issuer, "resolveDidOnLedger")) {
        await T("resolveDidOnLedger(issuer)", async () => issuer.resolveDidOnLedger(issuerDid));
  ...
  const report = {
    ...
    network: "VON_NETWORK_LOCAL",
    env: { GENESIS_FILE: process.env.GENESIS_FILE, ITER, WARMUP, RUN_TAG, MEASURE_RESOLVE },
    notes: {
      envelope_methods:
