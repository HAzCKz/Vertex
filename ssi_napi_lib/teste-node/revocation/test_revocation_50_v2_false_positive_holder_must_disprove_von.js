...
node teste-node/revocation/test_revocation_50_v2_false_positive_holder_must_disprove_von.js
...
/*
Teste E2E do fluxo v2 em cenário de falso positivo controlado.

Objetivo:
- emitir uma credencial não revogada com 10 janelas extras disponíveis para confirmação;
- induzir um falso positivo na janela principal;
- validar que, se o holder entregar apenas essa janela, o verifier responde needs_next_window;
- validar que, se o holder entregar também 2 janelas subsequentes e uma delas vier limpa,
  o verifier reclassifica o caso como false_positive_confirmed.
*/
...
async function writeDummyRevocations({
  ...
  const revocationKeys = Array.from({ length: count }, (_, idx) =>
    `dummy-fp-v2-${Date.now()}-${process.pid}-${idx}-${Math.random()}`
  );
  ...
  const resp = await fetch(`${baseUrl}/admin/revocations/v2`, {
    ...
    body: JSON.stringify({
      issuer_did: issuerDid,
      credential_record_id: `dummy-fp-batch-${Date.now()}`,
      revocation_keys: revocationKeys,
      window_starts: windowStarts,
      reason: "force-false-positive-for-v2-test",
      requested_by: "teste-node-revocation-50",
    }),
  });
...
(async () => {
  ...
  const issuerDb =
    process.env.WALLET_ISSUER || path.join(walletDir, "test_wallet_revocation_v2_false_positive_issuer.db");
  const holderDb =
    process.env.WALLET_HOLDER || path.join(walletDir, "test_wallet_revocation_v2_false_positive_holder.db");
  const verifierDb =
    process.env.WALLET_VERIFIER || path.join(walletDir, "test_wallet_revocation_v2_false_positive_verifier.db");
  ...
  try {
    console.log("🚀 TESTE REVOGAÇÃO 50: falso positivo v2 + ônus da prova do holder");
    ...
    const manifest = buildManifestAnchorFromEnvelope(issuerDid, BFILTER_BASE_URL, manifestAfterReset);
    ...
    console.log("4) Emitindo credencial revogável não revogada...");
    const schemaIdRev = await createAndRegisterSchema(
      genesisAbs,
      issuerDid,
      `SchemaRevV2FalsePositive_${Date.now()}`,
      `1.${nowSec()}`,
    ...
    const offerJson = await createCredentialOffer(credDefIdRev, `offer-v2-fp-${Date.now()}`);
    ...
    const issued = parseJsonSafe(issuedJson, "issued_revocable_v2_fp");

    const credentialId = `cred-v2-fp-${Date.now()}`;
    ...
    const bundleId = `bundle-v2-fp-${Date.now()}`;
    ...
    console.log("5) Holder preparando 2 versões da prova: só 1 janela e 3 janelas...");
    ...
    assert(proofSequenceOneWindow.proof_sequence.confirmation_proofs.length === 0, "não deveria haver janelas extras na primeira prova");

    const proofSequenceThreeWindowsJson = await buildPresentationRevocationProofV2(
      bundleId,
      PRIMARY_WINDOW_INDEX,
      ADDITIONAL_CONFIRMATION_WINDOWS,
      credentialId
    );
    ...
    assert(
      proofSequenceThreeWindows.proof_sequence.confirmation_proofs.length === ADDITIONAL_CONFIRMATION_WINDOWS,
      "deveriam existir 2 janelas subsequentes"
    );
    ...
    const presReq = {
      nonce: String(Date.now() * 1000 + 56789),
      name: "ProofReqRevocationV2FalsePositive",
      version: "0.1",
    ...
    console.log("7) Forçando falso positivo na janela principal e capturando a contraprova nas janelas seguintes...");
    ...
    assert(primaryWindowLooksRevoked, "não foi possível induzir falso positivo na janela principal dentro do limite");
    ...
    console.log("8) Validando a regra do ônus da prova: com só 1 janela, verifier deve pedir a próxima janela...");
    ...
    assert(isolatedOneWindowAfterFp.status.decision === "needs_next_window", "com só 1 janela o verifier deveria pedir a próxima janela");
    assert(isolatedOneWindowAfterFp.status.revoked === false, "com só 1 janela o status não deveria confirmar revogação");
    assert(isolatedOneWindowAfterFp.status.requires_more_windows === true, "com só 1 janela o verifier deveria pedir contraprova");
    assert(
      isolatedOneWindowAfterFp.status.trace.length >= 1 &&
        isolatedOneWindowAfterFp.status.trace[0].maybe_present === true,
      "a primeira janela deveria parecer revogada após o falso positivo"
    );

    console.log("9) Agora o holder fornece 2 janelas subsequentes e refuta o falso positivo...");
    ...
    assert(
      ...
      "com 3 janelas a decisão deveria ser false_positive_confirmed"
    );
    assert(isolatedThreeWindowsAfterFp.status.revoked === false, "após refutação do falso positivo o status não deveria ser revoked");
    assert(isolatedThreeWindowsAfterFp.status.accepted === true, "após refutação do falso positivo a decisão deveria ser aceita");
    assert(
      ...
      "o falso positivo deveria aparecer como true,false na sequência"
    );
    ...
    console.log("✅ OK: TESTE REVOGAÇÃO 50 passou.");
  ...
  console.error("❌ FALHA TESTE REVOGAÇÃO 50:", e && e.stack ? e.stack : e);
  process.exit(1);
});
