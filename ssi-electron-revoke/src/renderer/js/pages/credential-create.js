...
const CredentialCreatePage = (() => {
  ...
  const REVOCABLE_CONTROL_ATTRIBUTE_NAMES = [
    "seed",
    "start_time",
    "time_window",
    "unit_of_time",
  ...
  const REVOCABLE_CONTROL_ATTRIBUTE_KEYS = new Set(
    REVOCABLE_CONTROL_ATTRIBUTE_NAMES.map((name) => normalizeControlAttributeKey(name))
  );
  ...
  root.innerHTML = `
    <div class="card">
      ...
      <div
        id="create_revocable_schema_warning"
        class="small"
        style="display:none; margin:10px 0 16px; padding:12px; border:1px solid #f59e0b; border-radius:8px; background:#fffbeb; color:#92400e; white-space:pre-wrap;"
      ></div>
  ...
  function hasAllRevocableControlAttributes(attrNames) {
    const found = new Set(
      (Array.isArray(attrNames) ? attrNames : [])
        .map((name) => normalizeControlAttributeKey(name))
        .filter(Boolean)
    );
    return Array.from(REVOCABLE_CONTROL_ATTRIBUTE_KEYS).every((name) => found.has(name));
  }

  function buildRevocableCredentialWarning() {
    return [
      "Aviso: esta estrutura contém todos os atributos de controle de uma credencial revogável:",
      REVOCABLE_CONTROL_ATTRIBUTE_NAMES.join(", "),
      "Crie esta credencial pelo menu \"Criar Credencial Revogável\". A emissão pelo menu \"Criar Credencial\" foi bloqueada.",
    ].join("\n");
  }

  function showRevocableCredentialWarning() {
    const message = buildRevocableCredentialWarning();
    const el = $("#create_revocable_schema_warning");
    if (el) {
  ...
  function clearRevocableCredentialWarning() {
    const el = $("#create_revocable_schema_warning");
    if (el) {
      el.textContent = "";
  ...
  function updateFromImportedRequest(data) {
    ...
    if (matchedOfferPath) {
      $("#create_offer_file_path").value = matchedOfferPath;
  ...
  async function importRequestEnvelope() {
    ...
    // Evita que caminhos de uma importação anterior sejam reutilizados visualmente ou na emissão.
    $("#create_request_file_path").value = "";
    $("#create_offer_file_path").value = "";
  ...
  async function loadSchemaTemplate() {
    ...
    const attrNames = Array.isArray(r.data?.attrNames) && r.data.attrNames.length
      ? r.data.attrNames
      : Object.keys(tpl);
    if (hasAllRevocableControlAttributes(attrNames)) {
      showRevocableCredentialWarning();
      return;
    }

    clearRevocableCredentialWarning();
    $("#create_values_json").value = JSON.stringify(tpl, null, 2);
    Api.setStatus(`Atributos carregados (${attrNames.length}). Preencha os valores.`);
  }
  ...
  async function exportCredentialEnvelope() {
    ...
    if (hasAllRevocableControlAttributes(Object.keys(valuesObj || {}))) {
      const warning = showRevocableCredentialWarning();
      setOut({ where: "credentialCreate.exportCredentialEnvelope", blocked: true, reason: warning });
      return;
    }
    clearRevocableCredentialWarning();
