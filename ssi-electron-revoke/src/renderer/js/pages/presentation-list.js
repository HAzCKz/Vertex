...
const PresentationListPage = (() => {
  ...
  let pendingDeletePresentationId = "";
  ...
  root.innerHTML = `
    <div class="card">
      ...
      <div class="row" style="margin:8px 0 0 0">
        <button class="secondary" id="btn_pres_list_check_revocation">Verificar revogação</button>
      </div>

      <div id="pres_list_delete_modal" style="display:none; position:fixed; inset:0; background:rgba(15,23,42,.35); z-index:50; padding:24px;">
        <div style="max-width:560px; margin:8vh auto 0; background:#fff; border-radius:16px; padding:20px; box-shadow:0 20px 60px rgba(15,23,42,.18);">
          <h3 style="margin:0 0 10px 0;">Excluir apresentação</h3>
          <p id="pres_list_delete_modal_msg" class="small" style="white-space:pre-wrap; margin:0 0 16px 0;">
            Confirme a exclusão da apresentação selecionada.
          </p>
          <div class="row">
            <button class="secondary" id="btn_pres_list_delete_cancel">Cancelar</button>
            <button class="primary" id="btn_pres_list_delete_confirm">Excluir apresentação</button>
          </div>
        </div>
      </div>
      ...
      <div class="tableWrap">
        <table class="table" id="tbl_pres_list_revocation_windows">
          <thead>
            <tr>
              <th>Sub-proof</th>
              <th>Credencial</th>
              <th>CredDef ID</th>
              <th>Janelas Holder</th>
      ...
      <div class="row">
        <div class="input" style="min-width:620px">
          <label>Alerta de revogação</label>
          <textarea id="pres_list_revocation_alert" rows="5" readonly></textarea>
        </div>
      </div>
      ...
      <h3>Debug</h3>
      <pre id="pres_list_out" style="max-height:320px; overflow:auto; white-space:pre-wrap; word-break:break-word;">{}</pre>
    </div>
  `;
      ...
      tr.innerHTML = `
        ...
        <td>
          <div class="actions">
            ...
            <button data-act="delete" data-id="${it.id_local}">Excluir</button>
          </div>
        </td>
      `;
  ...
  function clearDetails() {
    renderRevocationWindowCoverage([]);
    renderRevealed([]);
    renderPredicates([]);
    $("#pres_list_revocation_alert").value = "";
    $("#pres_list_record_out").value = "";
  }

  function releaseHeavyState() {
    clearDetails();
    $("#pres_list_result").value = "";
    $("#pres_list_out").textContent = "{}";
    $("#tbl_pres_list_revocation_windows tbody").dataset.items = "[]";
    $("#tbl_pres_list_revealed tbody").dataset.items = "[]";
    $("#tbl_pres_list_predicates tbody").dataset.items = "[]";
    revealedItemsCache = [];
    predicateItemsCache = [];
  ...
  function closeDeleteModal() {
    pendingDeletePresentationId = "";
    $("#pres_list_delete_modal").style.display = "none";
  }

  function openDeleteModal(presentationIdLocal) {
    pendingDeletePresentationId = toStringSafe(presentationIdLocal).trim();
    if (!pendingDeletePresentationId) return;
    $("#pres_list_delete_modal_msg").textContent =
      `Confirme a exclusão da apresentação salva.\n\nID local: ${pendingDeletePresentationId}`;
    $("#pres_list_delete_modal").style.display = "block";
  }
  ...
  function formatWindowCoverageStatus(item) {
    if (item?.revoked === true) return "Revogada";
    if (item?.requires_more_windows === true) return "Exige mais janelas";
    if (item?.accepted === true) return "Aceita";
    if (item?.accepted === false) return "Não aceita";
    return "-";
  }

  function renderRevocationWindowCoverage(items, options = {}) {
    const tbody = $("#tbl_pres_list_revocation_windows tbody");
    tbody.innerHTML = "";
    ...
    if (!list.length) {
      ...
      td.colSpan = 9;
      td.className = "small";
      td.textContent = options?.checked
        ? "Nenhuma credencial revogável com janelas entregues pelo Holder foi encontrada nesta apresentação."
        : "Clique em Verificar revogação para ver as janelas de revogação entregues pelo Holder.";
      tr.appendChild(td);
      tbody.appendChild(tr);
      return;
    }
    ...
    list.forEach((item) => {
      const tr = document.createElement("tr");
      const fallbackLabel = `Sequência ${Number(item?.sequence_index ?? 0) + 1}`;
      appendTextCell(tr, item?.sub_proof_index);
      appendTextCell(tr, item?.credential_id_local || fallbackLabel, "mono");
      appendTextCell(tr, item?.cred_def_id, "mono");
      appendTextCell(tr, item?.delivered_window_count);
      appendTextCell(tr, item?.first_window_index);
      appendTextCell(tr, formatEpochSeconds(item?.first_window_start));
      appendTextCell(tr, item?.last_window_index);
      appendTextCell(tr, formatEpochSeconds(item?.last_window_start));
      appendTextCell(tr, formatWindowCoverageStatus(item));
      tbody.appendChild(tr);
    });
  }

  function buildRevocationAlertText(summaryItems) {
    const list = Array.isArray(summaryItems) ? summaryItems : [];
    const revoked = list.filter((it) => it?.revoked === true);
    if (!revoked.length) {
      return "Nenhuma credencial revogada foi detectada nesta apresentação salva.";
    }

    return revoked.map((it, index) => [
      `Credencial revogada ${index + 1}:`,
      `Sub-proof: ${firstNonEmpty(it?.sub_proof_index, "-")}`,
      `CredDef ID: ${firstNonEmpty(it?.cred_def_id, "-")}`,
      `Data de emissão: ${formatEpochSeconds(it?.issued_at) || "-"}`,
      `Data de revogação detectada: ${formatEpochSeconds(it?.revoked_window_start) || "-"}`,
      `Janela de revogação: ${firstNonEmpty(it?.revoked_window_index, "-")}`,
    ].join("\n")).join("\n\n");
  }

  function buildSnapshotRevocationAlertText(summaryItems) {
    const body = buildRevocationAlertText(summaryItems);
    return [
      "Snapshot salvo no momento em que a apresentação foi verificada/importada.",
      "Use o botão \"Verificar revogação\" para consultar o estado atual antes de confiar ou exportar.",
      "",
      body,
    ].join("\n");
  }

  function buildRevocationAlertFromLiveCheck(liveCheck, fallbackSummaryItems) {
    const live = liveCheck && typeof liveCheck === "object" ? liveCheck : null;
    const fallbackText = buildRevocationAlertText(fallbackSummaryItems);
    ...
    if (live.attempted && live.ok) {
      const items = Array.isArray(live.perCredentialStatus) ? live.perCredentialStatus : [];
      const revoked = items.filter((it) => it?.revoked === true);
      if (revoked.length > 0) {
        return [
          "Revalidação viva: foram detectadas credenciais revogadas nesta apresentação salva.",
          "",
          buildRevocationAlertText(revoked),
        ].join("\n");
      }
      if (live.requiresMoreWindows) {
        return [
          "Revalidação viva inconclusiva: a apresentação agora exige mais janelas de revogação.",
          "Ela não deve ser exportada sem nova complementação.",
        ].join("\n");
      }
      if (live.cryptographicValid === false || live.proofsVerified === false || live.accepted === false) {
        return [
          "Revalidação viva: a apresentação armazenada não foi aceita no estado atual.",
          `Política/resultado: ${firstNonEmpty(live.policy, "indisponível")}`,
        ].join("\n");
      }
      return "Revalidação viva concluída: nenhuma credencial revogada foi detectada no estado atual.";
    }

    if (live.attempted && !live.ok) {
      const reason = firstNonEmpty(live.message, "erro desconhecido");
      return [
        "Não foi possível revalidar a revogação em tempo real.",
        `Motivo: ${reason}`,
        "",
        "Exibindo abaixo o último snapshot salvo com a apresentação:",
        fallbackText,
      ].join("\n");
    }

    if (firstNonEmpty(live.reason) === "genesis_path_missing") {
      return [
        "Genesis path ausente: não foi possível revalidar a revogação em tempo real.",
        "Defina o Genesis Path no menu Ledger para atualizar este status.",
        "",
        "Exibindo abaixo o último snapshot salvo com a apresentação:",
        fallbackText,
      ].join("\n");
  ...
  async function loadPresentationDetails(presentationIdLocal) {
    ...
    const resp = await Api.presentation.getLocal({
      presentationIdLocal,
      compact: true,
      skipLiveRevocationCheck: true,
    });
    ...
    $("#pres_list_revocation_alert").value = buildSnapshotRevocationAlertText(
      d?.meta?.revocation_summary || []
    );
  ...
  async function verifySelectedPresentationRevocation() {
    if (!selectedPresentationId) {
      Api.setStatus("Selecione uma apresentação antes de verificar a revogação.");
      return;
    }
    if (revocationCheckInFlight) {
      Api.setStatus("A verificação de revogação já está em andamento.");
      return;
    }

    const presentationIdLocal = selectedPresentationId;
    const btn = $("#btn_pres_list_check_revocation");
    revocationCheckInFlight = true;
    if (btn) btn.disabled = true;

    try {
      Api.setStatus(`Revalidando revogação da apresentação ${presentationIdLocal}...`);
      const resp = await Api.presentation.verifyStoredRevocation({
        presentationIdLocal,
        genesisPath: firstNonEmpty(window.AppState?.genesisPath),
      });
      setOut({ where: "presentationList.verifyStoredRevocation", presentationIdLocal, resp });

      if (!resp?.ok) {
        Api.setStatus(`Erro verificando revogação da apresentação: ${resp?.error?.message || "erro desconhecido"}`);
        return;
      }

      const d = resp.data || {};
      $("#pres_list_revocation_alert").value = buildRevocationAlertFromLiveCheck(
        d?.liveRevocationCheck,
        d?.revocationSummary || []
      );
      renderRevocationWindowCoverage(d?.revocationWindowCoverage || [], { checked: true });

      const live = d?.liveRevocationCheck;
      if (live?.attempted && live?.ok) {
        if (live?.revoked) {
          Api.setStatus("Revogação revalidada: há credencial revogada nesta apresentação.");
          return;
        }
        if (live?.requiresMoreWindows) {
          Api.setStatus("Revogação revalidada: a apresentação exige mais janelas para conclusão.");
          return;
        }
        if (live?.cryptographicValid === false || live?.proofsVerified === false || live?.accepted === false) {
          Api.setStatus("Revogação revalidada: a apresentação não foi aceita no estado atual.");
          return;
        }
        Api.setStatus("Revogação revalidada: nenhuma credencial revogada foi detectada no estado atual.");
        return;
      }

      if (live?.attempted && !live?.ok) {
        Api.setStatus(`Não foi possível revalidar a revogação: ${firstNonEmpty(live?.message, "erro desconhecido")}`);
        return;
      }

      Api.setStatus("Esta apresentação salva não possui sequências de revogação para revalidação viva.");
    } finally {
  ...
  async function deletePresentationLocal() {
    if (!pendingDeletePresentationId) return;

    Api.setStatus(`Excluindo apresentação ${pendingDeletePresentationId}...`);
    const resp = await Api.presentation.deleteLocal(pendingDeletePresentationId);
    setOut({ where: "presentationList.deleteLocal", presentationIdLocal: pendingDeletePresentationId, resp });

    if (!resp?.ok) {
      Api.setStatus(`Erro excluindo apresentação: ${resp?.error?.message || "erro desconhecido"}`);
      return;
    }

    const deletedId = pendingDeletePresentationId;
    closeDeleteModal();
    ...
    await refreshList();
    Api.setStatus(`Apresentação excluída: ${deletedId}.`);
  }
  ...
  async function exportSelectedPresentationEnvelope() {
    ...
    const input = {
      ...
      expiresAtMs,
      metaObj,
      genesisPath: firstNonEmpty(window.AppState?.genesisPath) || null,
    };
  ...
  $("#btn_pres_list_check_revocation").addEventListener("click", () => {
    verifySelectedPresentationRevocation().catch(() => {});
  });
  ...
  $("#btn_pres_list_delete_cancel").addEventListener("click", closeDeleteModal);
  $("#btn_pres_list_delete_confirm").addEventListener("click", () => {
    deletePresentationLocal().catch(() => {});
  });
  $("#pres_list_delete_modal").addEventListener("click", (ev) => {
    if (ev.target?.id === "pres_list_delete_modal") closeDeleteModal();
  });
  root.addEventListener("keydown", (ev) => {
    if (ev.key === "Escape" && $("#pres_list_delete_modal").style.display !== "none") {
      closeDeleteModal();
    }
  });
  ...
  window.addEventListener("app:pagechange", (ev) => {
    ...
    if (from === "page-presentation-list" && to !== "page-presentation-list") {
      releaseHeavyState();
    }
  });
