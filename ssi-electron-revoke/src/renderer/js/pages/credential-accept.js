...
const CredentialAcceptPage = (() => {
  ...
  async function refreshImportedOfferAttributes() {
    ...
    if (!r?.ok) {
      const msg = toStringSafe(r?.error?.message).trim();
      const staleCredDefMsg = msg && (
        msg.toLowerCase().includes("data is null")
        || msg.toLowerCase().includes("não encontrada")
        || msg.toLowerCase().includes("nao encontrada")
      )
        ? `${msg} A oferta foi gerada com uma CredDef que não está mais válida no ledger. Gere uma nova oferta usando a CredDef atualmente publicada.`
        : (r?.error?.message || "erro desconhecido");
      setOfferAttributes(
        [],
        `Não foi possível carregar os atributos: ${staleCredDefMsg}.`
      );
      return;
    }
    ...
    const resolvedCredDefId = toStringSafe(r.data?.credDefIdResolved).trim();
    const aliasNote = r.data?.credDefAliasUsed && resolvedCredDefId
      ? ` CredDef publicada usada: ${resolvedCredDefId}.`
      : "";
    setOfferAttributes(
      entries,
      `${entries.length} atributo(s) carregado(s) do ${sourceLabel}.${aliasNote}`
    );
  }
