...
const CredentialOfferPage = (() => {
  ...
  function parseCredDefIdParts(credDefId) {
    const id = toStringSafe(credDefId).trim();
    const marker = ":3:CL:";
    ...
    const schemaRef = lastColon >= 0 ? rest.slice(0, lastColon) : rest;
    const tag = lastColon >= 0 ? rest.slice(lastColon + 1) : "";
    return { issuerDid, schemaRef, tag };
  }

  function splitTagBase(tag) {
    const raw = toStringSafe(tag).trim();
    const match = raw.match(/^(.*)_(\d{8,})$/);
    if (!match) return { baseTag: raw, suffixNum: -1 };
    return {
      baseTag: toStringSafe(match[1]).trim(),
      suffixNum: Number.parseInt(match[2], 10),
    };
  }

  function credDefEquivalenceKey(credDefId) {
    const parts = parseCredDefIdParts(credDefId);
    if (!parts) return toStringSafe(credDefId).trim();
    const tagParts = splitTagBase(parts.tag);
    return `${parts.issuerDid}|${parts.schemaRef}|${tagParts.baseTag}`;
  }

  function compareCredDefFreshness(leftId, rightId) {
    const left = parseCredDefIdParts(leftId);
    const right = parseCredDefIdParts(rightId);
    if (!left || !right) return 0;

    const leftTag = splitTagBase(left.tag);
    const rightTag = splitTagBase(right.tag);
    const leftSuffix = Number.isFinite(leftTag.suffixNum) ? leftTag.suffixNum : -1;
    const rightSuffix = Number.isFinite(rightTag.suffixNum) ? rightTag.suffixNum : -1;
    if (leftSuffix !== rightSuffix) return leftSuffix - rightSuffix;

    return toStringSafe(leftId).trim().localeCompare(toStringSafe(rightId).trim());
  }
  ...
  async function refreshOptions() {
    ...
    try {
      ...
      records = listB.length ? listB : listA;
    } catch (_) {
      records = [];
    }
    ...
    credDefOptions = records
      ...
      .filter((it) => !!it.credDefId)
      .forEach((it) => {
        const key = credDefEquivalenceKey(it.credDefId);
        const prev = newestByGroup.get(key);
        if (!prev || compareCredDefFreshness(prev.credDefId, it.credDefId) < 0) {
          newestByGroup.set(key, it);
        }
      });
    credDefOptions = Array.from(newestByGroup.values())
      .sort((left, right) => compareCredDefFreshness(right.credDefId, left.credDefId));
    applyCredDefFilter();

    const manualCredDefId = toStringSafe($("#offer_creddef_id").value).trim();
    const visibleCredDefIds = new Set(credDefOptions.map((c) => toStringSafe(c.credDefId).trim()));
    if (manualCredDefId && visibleCredDefIds.has(manualCredDefId)) {
      $("#sel_offer_creddef").value = manualCredDefId;
    } else if (credDefOptions.length >= 1) {
      const firstCredDefId = credDefOptions[0].credDefId;
      $("#sel_offer_creddef").value = firstCredDefId;
      if (!manualCredDefId || !visibleCredDefIds.has(manualCredDefId)) {
        $("#offer_creddef_id").value = firstCredDefId;
      }
    }
