...
const Api = {
  ...
  credCreateRevocable: {
    setupKVector: (input) => window.ssi.credCreateRevocable.setupKVector(input),
    writeKVectorOnLedger: (input) => window.ssi.credCreateRevocable.writeKVectorOnLedger(input),
    checkManifestOnLedger: (input) => window.ssi.credCreateRevocable.checkManifestOnLedger(input),
    anchorManifestOnLedger: (input) => window.ssi.credCreateRevocable.anchorManifestOnLedger(input),
    readLedgerSetup: (input) => window.ssi.credCreateRevocable.readLedgerSetup(input),
    exportCredentialEnvelope: (input) => window.ssi.credCreateRevocable.exportCredentialEnvelope(input),
  },

  credReceive: {
    previewEnvelope: (input) => window.ssi.credReceive.previewEnvelope(input),
  ...
  credential: {
    ...
    delete: (credentialIdLocal) => window.ssi.credential.delete(credentialIdLocal),
  },

  credRevoke: {
    listIssuedRevocable: (input) => window.ssi.credRevoke.listIssuedRevocable(input),
    getIssuedSummary: (input) => window.ssi.credRevoke.getIssuedSummary(input),
    deleteIssued: (input) => window.ssi.credRevoke.deleteIssued(input),
    preflight: (input) => window.ssi.credRevoke.preflight(input),
    execute: (input) => window.ssi.credRevoke.execute(input),
  },

  revocationVerify: {
    getHolderBundle: (input) => window.ssi.revocationVerify.getHolderBundle(input),
    buildProofSequence: (input) => window.ssi.revocationVerify.buildProofSequence(input),
    verifyProofSequence: (input) => window.ssi.revocationVerify.verifyProofSequence(input),
    fullScan: (input) => window.ssi.revocationVerify.fullScan(input),
  },
  ...
  presentation: {
    ...
    getLocal: (input) => window.ssi.presentation.getLocal(input),
    verifyStoredRevocation: (input) => window.ssi.presentation.verifyStoredRevocation(input),
    deleteLocal: (presentationIdLocal) => window.ssi.presentation.deleteLocal(presentationIdLocal),
